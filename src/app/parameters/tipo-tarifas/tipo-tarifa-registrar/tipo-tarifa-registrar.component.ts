...
@Component({
  selector: 'app-tipo-tarifa-registrar',
  templateUrl: './tipo-tarifa-registrar.component.html',
  styleUrls: ['./tipo-tarifa-registrar.component.scss']
})
export class TipoTarifaRegistrarComponent implements OnInit {
  ...
  constructor(  private empresaTipoTarifaService: EmpresaTipoTarifaService,
...
//------------------------------------------------------------------------------------------
  ngOnInit(): void {
      this.suscription= this.empresaTipoTarifaService.obtenerDatosRegistroObservable$().subscribe(data => {
    ...
    ActualizarRegistro(){
      ...
      this.formulariosService.compareAndGeneratePatch(jsonPatch, this.form.value, this.originalFormValues);
        ...
        this.sharedService.patch(this.endPoint, this.idRegistro, jsonPatch).subscribe(
          response => {
            this.empresaTipoTarifaService.obtenerListadoRegistros('/' + this.endPoint);
