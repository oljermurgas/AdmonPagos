...
@Component({
  selector: 'app-tipo-empresa-sector-registrar',
  templateUrl: './tipo-empresa-sector-registrar.component.html',
  styleUrls: ['./tipo-empresa-sector-registrar.component.scss']
})
export class TipoEmpresaSectorRegistrarComponent implements OnInit {
  suscription: Subscription = new Subscription;
  form: FormGroup; 
  originalFormValues: any;
  idRegistro: number =0;
  endPoint="TipoEmpresaSector"

  constructor(  private empresaSectorService: EmpresaSectorService,
...
//------------------------------------------------------------------------------------------
  ngOnInit(): void {
      this.suscription= this.empresaSectorService.obtenerDatosRegistroObservable$().subscribe(data => {
    ...
    ActualizarRegistro(){
      ...
      this.formulariosService.compareAndGeneratePatch(jsonPatch, this.form.value, this.originalFormValues);
        ...
        this.sharedService.patch(this.endPoint, this.idRegistro, jsonPatch).subscribe(
          response => {
            this.empresaSectorService.obtenerListadoRegistros('/' + this.endPoint);
