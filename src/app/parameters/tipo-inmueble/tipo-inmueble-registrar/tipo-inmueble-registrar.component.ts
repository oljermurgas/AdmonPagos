...
@Component({
  selector: 'app-tipo-inmueble-registrar',
  templateUrl: './tipo-inmueble-registrar.component.html',
  styleUrls: ['./tipo-inmueble-registrar.component.scss']
})
export class TipoInmuebleRegistrarComponent implements OnInit {
  suscription: Subscription = new Subscription;
  form: FormGroup; 
  originalFormValues: any;
  idRegistro: number =0;
  endPoint="TipoInmueble"

  constructor(  private inmuebleTipoService: InmuebleTipoService,
...
//------------------------------------------------------------------------------------------
  ngOnInit(): void {
      this.suscription= this.inmuebleTipoService.obtenerDatosRegistroObservable$().subscribe(data => {
        console.log("Llamado:", data);
    ...
    ActualizarRegistro(){
      ...
      this.formulariosService.compareAndGeneratePatch(jsonPatch, this.form.value, this.originalFormValues);
        ...
        this.sharedService.patch(this.endPoint, this.idRegistro, jsonPatch).subscribe(
          response => {
            this.inmuebleTipoService.obtenerListadoRegistros('/' + this.endPoint);
